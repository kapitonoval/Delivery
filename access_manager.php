...
if($result->num_rows > 0){
	while($item = $result->fetch_assoc()){
		   ...
		   for($i = -5 ; $i < 10 ; $i++ ){
			...
			$content.= '<td valign="top" data-id="'.$item['id'].'" data-name="target" class="'.$red_class.' redactorTD" onclick="changeTypeCell(this);">'.($item['target']).'</td>';
			...
			$content .='<div class="table" style="height:100%">';
					    ...
					    foreach ($task as $key => $value){		
							if(isset($value['id_parent']) && $value['id_parent']==$item['id']){
								if($_SESSION['access']['user_id']==$value['id_manager'] /*|| $_SESSION['access']['access']=='1'*/){
									...
									$content .= '" style="vertical-align:top; width:27px; cursor:pointer;';
									...
									//колонка _2_
									$content .='<div class="cell" style="width:325px; "><textarea style=\'font-family:Gotham, "Helvetica Neue", Helvetica, Arial, sans-serif; font-size:12px; background:none; width:95%; height:7px\' name="" class="action_text" id="task_'.$value['id'].'" onKeyDown="textareaChangeMin(this)" onKeyPress="textareaChangeMin(this)" onKeyUp="textareaChangeMin(this)">'.$value['actions'].'</textarea><div style="width:100%; float:left:">'.$value['date_log'].'</div></div>';
									//колонка _3_
									...
									$content .= '<div class="cell" onClick="changeTypeCell(this,\'addRow_point\')"  data-name="second_point" data-id="'.$value['id'].'" style="width:80px;cursor:pointer; vertical-align:top; text-align:left; font-size:12px; color:grey">'.$value['second_point'].'</div>';	
									}else{
									$content .= '<div class="cell" style="width:80px; vertical-align:top; text-align:left; font-size:12px; color:grey">'.$value['second_point'].'</div>';
									}
									//колонка _4_
									$create_time = ((isset($value['create_time']) && $value['create_time'] != '0000-00-00 00:00:00')?'<br>'.date('d.m.Y H:i',strtotime($value['create_time'])):"");
									$content .='<div class="cell" style="vertical-align:top;"><div style="float:left;width:90px;">'.$value['name'].' '.mb_substr($value['last_name'], 0, 2).'.'.$create_time.'</div></div>';
									
									//колонка _5_
									$content .='<div class="cell" onClick="cleanThisePosition(this)" style="vertical-align:top; cursor:pointer; padding:5px;"><div class="BtnAddOrDel"><img src="img/del2.png" style="margin:0px 5px 0 0; "></div></div>
									</div>';								
									}else{
									...
									//колонка _2_
									$content .= '<div class="cell" style="width:340px; padding:5px; min-height:80px;"><div style=\'border:none;font-family:Gotham, "Helvetica Neue", Helvetica, Arial, sans-serif; font-size:12px; width:95%; min-height:20px; height:95%\'>'.$value['actions'].'</div><div style="width:100%; float:left; padding:0 0 0 5px; font-size:10px ">'.$value['date_log'].'</div></div>';
									//колонка _3_
									$content .= '<div class="cell" style="width:80px; font-size:12px;color:grey">'.$value['second_point'].'</div>';
									//колонка _4_
									$create_time = ((isset($value['create_time']) && $value['create_time'] != '0000-00-00 00:00:00')?'<br>'.date('d.m.Y  H:i',strtotime($value['create_time'])):"");
									
									$content .= '<div class="cell"><div style="float:left;width:90px;">'.$value['name'].' '.mb_substr($value['last_name'], 0, 2).'. '.$create_time.'</div></div>';
									//колонка _5_
									$content .= '<div class="cell" style="padding:5px;"><div style="padding: 0 5px 0 5px; width:25px;"></div></div></div>';
									$s_row++;
								}								
							}
						}
						...
						if($s_row>0 && $s_row==$s_checked){//строки есть и все выделены
							$content .='<div class="cell" style="width:27px;background:url(img/check.png) no-repeat; cursor:pointer;  border-right:1px solid #ddd" onClick="checkS(this)"> <input type="checkbox" class="checking_all" onClick="checkAllRow(this)"  style="display:none" checked></div>';
						}else if($s_row>0 && $s_row!=$s_checked && $s_checked>0){//строки есть и не все выделены
							$content .='<div class="cell" style="width:27px;background:url(img/minus.png) no-repeat; cursor:pointer; border-right:1px solid #ddd;" onClick="checkS(this)"> <input type="checkbox" class="checking_all" onClick="checkAllRow(this)"  style="display:none" ></div>';///8888888
						}else if($s_row>0 && $s_checked==0){//строки есть и ниодна невыделена 
							$content .='<div class="cell" style="width:27px;background:url(img/unch.png) no-repeat; cursor:pointer; border-right:1px solid #ddd;" onClick="checkS(this)"> <input type="checkbox" class="checking_all" onClick="checkAllRow(this)"  style="display:none" ></div>';///8888888
						}else{//строк нет
							$content .='<div class="cell" style="width:27px;background:none; cursor:pointer; border-right:1px solid #ddd; width:27px;"> <input type="checkbox" class="checking_all" onClick="checkAllRow(this)"  style="display:none" ></div>';///8888888
							}						
				...
				<input name="'.$item['id'].'" type="button" style=" background:url(img/del.png) no-repeat; width: 27px; height:27px; border:none; cursor:pointer" value="" onClick="delBigRow(this)" style="width:60px;">
					</td>
				  </tr>';
	    }
	}
...
<script type="text/javascript" src="dostavka_manager_js.js"></script>
    ...
    <tr>
      <td style="border:none;"><table width="900" id="myTable" border="1" cellpadding="0" cellspacing="0" align="center">
          <tr class="nodrop nodrag">
            ...
            <td colspan="2" width="70" align="center" style=" background-color:#EEEEEE;"><a href="/dostavka_new/index.php?day=<?php  echo $day.'.'.$month.'.'.$year; ?>" style="display:block;font-size:14px;height:27px;padding-top:6px;" onMouseOver="this.parentNode.style.backgroundColor='#D5D5D5'"  onMouseOut="this.parentNode.style.backgroundColor='#EEEEEE'">сводная карта</a></td>
...
2.   Проект111, Оазис  - ездим туда по пн,ср,пт</strong></td>
          ...
          <tr class="nodrop nodrag">
            ...
            <td class="table_top" width="200px"><p>Адрес/Контактное лицо/Телефон</p></td>
          ...
          <?php  echo  $content; ?>
            ...
            <tr class="nodrop nodrag" id="endBigRow">
              ...
              <td align="center"></td>
              <!--<td align="center" onClick="selectTheAdress(this)" id="add_address"><input name="add_data" type="button" id="addBigRow" style=" background:url(img/add.png) no-repeat; width:27px; height:27px; border:none; cursor:pointer" value=""></td>-->
              <td align="center" id="add_new_address_id" onClick="add_new_address(this)"><input name="add_data" type="button" style=" background:url(img/add.png) no-repeat; width:27px; height:27px; border:none; cursor:pointer" value=""></td>
...
<!-- шаблоны для javascript -->
    ...
    <div id="template_add_min_row">
    	<div class="row">
        	<div class="cell" style=" vertical-align:top; width:27px;border-right:1px solid #dddddd; cursor:pointer;">
            	<input type="checkbox" onClick="endThis(this)" class="statusTask" style="display:none">
            </div>
            <div class="cell" style="width: 340px;">
                ...
                <textarea class="small_row_txt"  onfocus="moveCursorToEnd(this)" height="7px" style="width: 95%;" name="" onKeyDown="textareaChangeMin(this)" onKeyPress="textareaChangeMin(this)" onKeyUp="textareaChangeMin(this)"></textarea>
			</div>
            <div class="cell" style="vertical-align:top;width:100px;font-size:14px; color:#808080"></div>
            ...
            <div class="cell" style="vertical-align:top; cursor:pointer; padding:5px; width:45px;">
                <div class="BtnAddOrDel" onClick="delMyDOMSmallRow(this)" id="delMyMinRow" style="font-size:12px; padding: 0 5px 0 5px;">
                	<img src="img/del2.png" style="margin:0px 5px 0 0; ">
                </div>
            </div>
		</div>
    </div>
